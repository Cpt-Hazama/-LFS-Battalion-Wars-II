...
ENT.DMG = BWII_DMG_MISSILE
ENT.DMGDist = 180
ENT.Colors = {r=30,g=30,b=30}
...
if SERVER then
	...
	function ENT:Initialize()	
		...
		if self:GetNWBool("SolarEffects") then
			self.Materials = {
	...
	function ENT:Think()
		...
		self.SmokeEffect1:SetColor(30,30,30,255)
	...
	function ENT:doFX( pos )
		...
		if self:GetDirtyMissile() then
			...
			local particle = emitter:Add( Material("effects/ar2_altfire2"), pos ) // Glow
			if particle then
				particle:SetVelocity( -self:GetForward() *300 +self:GetUp() *math.Rand(-450,450) +self:GetRight() *math.Rand(-450,450) +self:GetVelocity())
			...
			local particle = emitter:Add( Material("sprites/light_glow02_add"), pos ) // Glow
			if particle then
